...
# from DetectedFrameUrlProducer import produce_message
...
output_file_base = "../resources/json/frame"
...
def raise_flag(category: str, img, event_type: str, timestamp: str, frame: str,
    ...
    generate_json(category, safety_score, description, event_type, timestamp, frame,
...
def anomaly_detector(img, box, x1: int, y1: int, x2: int, y2: int, Id: int, currentClass: str, conf: int):
    ...
    if currentClass in ('NO-Safety Vest', 'NO_Hardhat', 'NO-Mask'):
        ...
        if Id not in violation_dict.keys() and Id != None:
            ...
            generate_json("Non-Alert", safety_score, " ", "PPE Violation", "2024-07-01T14:23:45Z", frame_number,
...
def detect(address: str, video_mode: str, object_counter_requirement: bool, class_names: List):
    ...
    output_image_path = f"../resources/detected_frames/frame_{frame_number}.png"
...
def process_video(video_path: str, object_counter_requirement: bool, class_names: List):
    ...
    while True:
        ...
        print(detect(temp_frame_path, "PNG", object_counter_requirement, class_names))
        frame_number += 1
...
def consume_frames():
    ...
    while True:
        ...
        frame_path = os.path.join('../resources/frames', frame_url)

        if os.path.exists(frame_path):
            print(f'Frame exists at: {frame_path}')
            detect(frame_path, "PNG", True, className)
        else:
